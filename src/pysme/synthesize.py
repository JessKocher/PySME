...
def _temporary_brackett_convolution_env(sme):
    ...
    env_name = "PYSME_H_STARK_CONVOLUTION"
    mode = _brackett_convolution_mode(sme)
...
def _brackett_convolution_mode(sme):
    """Return the canonical native/cache mode for an SME-like object."""
    mode = getattr(sme, "h_stark_convolution", "legacy")
    if mode is None:
...
def _compute_almax_lineinfo_for_sme(sub_sme):
    ...
    if accrt is None:
        accrt = sub_sme.accrt
    # ALMAX is derived from the same opacity calculation as synthesis.  Keep
    # the mode scoped across the complete native workflow, including worker
    # processes used for chunked preselection.
    with _temporary_brackett_convolution_env(sub_sme):
...
def _compute_linelist_hash(linelist, h_stark_convolution="legacy"):
    """Stable hash for line-level cache isolation (order-sensitive).

    Keep the legacy digest unchanged for compatibility with existing cache
    files.  The opt-in convolution path gets a namespaced digest so a cache
    generated by one Stark construction can never be selected for the other.
    """
    hasher = hashlib.sha1()
    ...
    if str(h_stark_convolution).lower() == "convolution":
        hasher.update(b"h_stark_convolution=convolution")
...
def _load_lineinfo_cache_file(
    path, method, linelist_hash, n_lines_total, h_stark_convolution="legacy"
):
    with np.load(path) as z:
        ...
        if "h_stark_convolution" in z:
            file_mode = str(np.asarray(z["h_stark_convolution"]).item()).lower()
            expected_mode = str(h_stark_convolution).lower()
            if file_mode != expected_mode:
                raise ValueError(
                    f"{path} h_stark_convolution mismatch: "
                    f"{file_mode} != {expected_mode}"
                )
        elif str(h_stark_convolution).lower() == "convolution":
            # Legacy cache files predate the mode metadata.  They remain
            # valid for the legacy path, but are never valid for convolution.
            raise ValueError(f"{path} has no h_stark_convolution metadata")
...
class Synthesizer:
    ...
    def synthesize_spectrum(
        ...
        if line_select_method == "cdr":
            ...
            cdr_mode = getattr(sme.linelist, "cdr_paras_h_stark_convolution", None)
            ...
            stale_model = cdr_mode is None and current_brackett_mode == "convolution"
            if cdr_mode is not None and str(cdr_mode).lower() != current_brackett_mode:
                stale_model = True
            if cdr_paras is None:
                stale_model = True
            ...
            stale_model = almax_mode is None and current_brackett_mode == "convolution"
        ...
        if updateStructure:
            ...
            if passLineList and self.update_cdr_switch:
                ...
                if len(central_depth[s]) > 0:
                    ...
                    sme.linelist.cdr_paras_h_stark_convolution = _brackett_convolution_mode(sme)
    ...
    def synthesize_segment(
        ...
        # Scope all native hydrogen-profile switches to this synthesis call.
        with (
            _temporary_smelib_experimental_env(sme),
            _temporary_brackett_convolution_env(sme),
        ):
    ...
    def _interpolate_or_compute_and_update_linelist(
        ...
        h_stark_convolution = _brackett_convolution_mode(sme)
        linelist_hash = _compute_linelist_hash(sme.linelist, h_stark_convolution)
                ...
                if simplex_index >= 0:
                    ...
                    for pt in vertices:
                        ...
                        data = _load_lineinfo_cache_file(
                            ...
                            h_stark_convolution=h_stark_convolution,
                        )
                    ...
                    if method == "cdr":
                        sme.linelist.cdr_paras_h_stark_convolution = h_stark_convolution
                    return
        ...
        if os.path.exists(full_path) and not cdr_grid_overwrite:
            ...
            np.savez_compressed(
                ...
                h_stark_convolution=np.array(h_stark_convolution),
